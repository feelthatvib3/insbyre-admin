...
import { useUpdateProductMutation } from 'shared/api/product/product.mutations';
...
export function EditProductForm({ categories, product }: EditProductFormProps) {
  ...
  const isProductSame = !form.formState.isDirty;
  ...
  async function onSubmit(
    values: Omit<ProductWithoutIdAndDate, 'thumbnail' | 'images'> & { images: (File | string)[] }
  ) {
    setError(null);
    setIsUploading(true);

    try {
      const uploads = values.images.map(async (item) => {
        ...
        const { error } = await supabase.storage.from('images').upload(filePath, item);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center gap-x-2">
          <Button
            disabled={isUploading || isPending || isProductSame}
            type="submit"
            size="lg"
            variant="primary"
          >
